...
int main (int argc, char ** argv) {
  ...
  ofstream outputFile("allowed_particles2.txt");
  ...
  while ( mixer.next_event() && ( iev < nEvent || nEvent == -1 ) )
  {
    ...
    for (const auto& particle : selected_parti) {
      outputFile << ", P_x: " << particle.px() << ", P_y: " << particle.py() << ", P_z: " << particle.pz();
      outputFile << ", MassPar: " << particle.m() << ", Pt_part: "<< particle.pt() << endl;
     }
    ClusterSequenceArea csSig(selected_parti, jet_def, area_def);

    jetCollection jetCollectionSig(sorted_by_pt(jet_selector(csSig.inclusive_jets(10.0))));
